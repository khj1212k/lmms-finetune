...
DATA=msmt17_train_full_55                               # model id; pick on by running `python supported_models.py`
TRAIN_DATA_PATH=./example_data/msmt17_train_full_55.jsonl  # path to the training data json file
EVAL_DATA_PATH=./example_data/msmt17_val_full_55.jsonl    # path to the evaluation data json file (optional)
IMAGE_FOLDER=./example_data                        # path to the image root folder; if provided, the image paths in the json should be relative
...
Q_LORA=True                                            # whether use q-lora for llm; only effective when `USE_LORA` is True
...
RUN_ID=${NUM_GPUS}gpu_${NUM_EPOCHS}epo_${PER_DEVICE_BATCH_SIZE}b_${MODEL_ID}_${DATA}_lora-${USE_LORA}_qlora-${Q_LORA}_visionlora-${USE_VISION_LORA}     # a custom run id that determines the checkpoint folder and wandb run name
